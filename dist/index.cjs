...
// revert     { names }
const EVENTS = ['created', 'filtered', 'loaded', 'migration', 'up', 'down', 'active', 'revert'];
...
const APP_PKG = readJSONSync(require$$1.join(process.cwd(), 'package.json'));
const MYGRA_CONFIG_DIR = require$$1.join(os.homedir(), '.mygra');
const MYGRA_DEFAULT_PATH = require$$1.join(process.cwd(), 'mygra');
const MYGRA_CONFIG_PATH = require$$1.join(os.homedir(), '.mygra');
...
function readFileAsync(path) {
    return new Promise((res, rej) => {
        lib.readFile(path, 'utf8', (err, data) => {
            if (err)
                rej(err);
            return res(data);
        });
...
function readJSONSync(path, defaults = {}) {
    if (!lib.existsSync(path))
        return defaults;
    return JSON.parse(lib.readFileSync(path, 'utf-8'));
}
/**
...
function colorize(str, ...styles) {
    return styles.reduce((a, c) => {
        if (!colors__default['default'][c])
            return a;
        return colors__default['default'][c](a);
    }, str);
}
/**
 *
...
class Mygra extends events.EventEmitter {
    constructor(options = {}) {
        super();
    ...
    /**
      ...
      */
    async getTemplates(templates = this.templates) {
        const templatesDir = require$$1.join(this.directory, 'templates');
        const hasTemplates = require$$0$2.existsSync(templatesDir);
     ...
     */
    async getFilenames() {
        let files = await glob__default['default'](`${this.directory}/migrations/**/*${this.extension}`, { onlyFiles: true });
        files = files.sort().reverse().map(f => require$$1.resolve(f));
        return files;
    }
    /**
    ...
    async import(filename) {
        const file = await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(filename)); });
    ...
    async load(filenames) {
        ...
        const migrations = await Promise.all(proms);
     ...
     */
    async up(nameOrLevels, preview = false) {
        let files;
        ...
        try {
            if (typeof nameOrLevels === 'number' || nameOrLevels === '*')
                ...
                files = await this.filter(nameOrLevels, 'up');
            ...
            if (!files.length) {
                ...
                for (const [i, file] of migrations.entries()) {
                    const name = require$$1.parse(files[i]).name;
                    this.emit('up', { name });
                    await require$$4.promisify(file.up)(this.connection);
                    migrated.push(file);
                    count++;
                }
                result = {
     ...
     * @returns status, count affected and message.
     */
    async down(nameOrLevels, preview = false) {
        let files;
        let result = { type: 'up', ok: false, message: 'Unknown', count: 0, names: [] };
        let count = 0;
        const migrated = [];
        try {
            ...
            if (!files.length) {
                ...
                for (const [i, file] of migrations.entries()) {
                    const name = require$$1.parse(files[i]).name;
                    this.emit('down', { name });
                    await require$$4.promisify(file.down)(this.connection);
                    migrated.push(file);
                    count++;
                }
                result = {
     ...
     * @returns status, count affected and message.
     */
    async revert(migrations, dir, preview = false) {
...
exports.APP_PKG = APP_PKG;
exports.EVENTS = EVENTS;
exports.MYGRA_CONFIG_DIR = MYGRA_CONFIG_DIR;
exports.MYGRA_CONFIG_PATH = MYGRA_CONFIG_PATH;
exports.MYGRA_DEFAULTS = MYGRA_DEFAULTS;
exports.MYGRA_DEFAULT_PATH = MYGRA_DEFAULT_PATH;
exports.Mygra = Mygra;
exports.PKG = PKG;
